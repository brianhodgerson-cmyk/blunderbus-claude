...
### Service status (running as Docker on Cortex)
```bash
ssh cortex "docker ps --filter name=vaultwarden --format '{{.Names}}: {{.Status}}'"
```
...
```bash
ssh cortex "docker logs --tail=30 --since=24h vaultwarden 2>&1 | grep -i 'error\|warn\|fail'"
```
